...
class ProductImporter extends Importer implements WithMapping
{
    ...
    public function examples(): array
    {
        ...
        $examples = [
            [
                ...
                'seo_image' => 'https://example.com/product-seo.jpg',
                'seo_index' => 'index',
    ...
    public function productResults(Collection $products): array
    {
        ...
        foreach ($products as $product) {
            ...
            $result['seo_image'] = is_array($seoMeta) ? ($seoMeta['seo_image'] ?? '') : '';
    ...
    protected function updateProduct(Product $product, array $row, Request $request): Product
    {
        ...
        if (array_key_exists('seo_meta', $row)) {
            $product->saveMetaData('seo_meta', $row['seo_meta']);
        }
    ...
    public function mapLocalization(array $row): array
    {
        ...
        $seoImage = trim((string) Arr::get($row, 'seo_image'));
        if ($seoImage && strcasecmp($seoImage, 'REMOVE') !== 0) { $seoImage = Arr::first($this->getImageURLs([$seoImage])); }
        $seoIndex = Arr::get($row, 'seo_index', 'index');

        if ($seoTitle || $seoDescription || $seoImage || ($seoIndex && $seoIndex !== 'index')) {
