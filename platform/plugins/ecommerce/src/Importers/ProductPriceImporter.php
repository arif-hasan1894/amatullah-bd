...
class ProductPriceImporter extends Importer
{
    ...
    {
        return 'plugins/ecommerce::products.partials.product-price-import-extra-fields';
    }
    ...
    public function columns(): array
    {
        return [
            ...
            ImportColumn::make('price')
                ->nullable()
                ->rules(
                    $this->updateExisting ? ['nullable', 'numeric'] : ['required', 'numeric'],
    ...
    public function handle(array $data): int
    {
        ...
        return Product::withoutTimestamps(function () use ($data): int {
            $updated = 0;

            foreach ($data as $row) {
                $product = Product::query()->find($row['id'] ?? null);
                ...
                $changes = [];
                foreach (['cost_per_item', 'price', 'sale_price'] as $field) {
