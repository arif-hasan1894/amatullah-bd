...
class ProductSpecificationImporter extends Importer implements WithMapping
{
    ...
    {
        return 'plugins/ecommerce::products.partials.product-specification-import-extra-fields';
    }
    ...
    public function handle(array $data): int
    {
        ...
        foreach ($data as $row) {
            ...
            $specificationTableName = trim((string) ($row['specification_table'] ?? ''));

            if ($this->updateExisting) {
                if ($specificationTableName !== '') {
                    $specificationTableId = $this->resolveSpecificationTableId($specificationTableName);

                    if ($specificationTableId !== null) {
                        $product->specification_table_id = $specificationTableId;
                        $product->saveQuietly();
                    }
                }
            } else {
                $specificationTableId = $specificationTableName !== ''
                    ? $this->resolveSpecificationTableId($specificationTableName)
                    : null;
    ...
    protected function syncSpecifications(Product $product, string $specificationsString): void
    {
        ...
        if (! empty($pivotData)) {
            if ($this->updateExisting) {
                $product->specificationAttributes()->syncWithoutDetaching($pivotData);
            } else {
