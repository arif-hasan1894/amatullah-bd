...
<?php endif; ?>
        ...
        <?php if(!empty($cartItem->options['options'])): ?>
            <?php echo render_product_options_html($cartItem->options['options'], $product->front_sale_price); ?>

        <?php endif; ?>

        <?php if(EcommerceHelper::isTaxEnabled() && EcommerceHelper::isDisplayItemTaxAtCheckout() && $cartItem->taxRate > 0 && $cartItem->taxTotal > 0 && ! $cartItem->options->get('price_includes_tax', false)): ?>
            <p class="mb-0 ec-checkout-item-tax">
                <small class="text-muted">
                    <?php echo e(__('Tax')); ?>: <?php echo e(format_price($cartItem->taxTotal)); ?>

                    <?php if(EcommerceHelper::isDisplayCheckoutTaxInformation() && $cartItem->options && $cartItem->options->taxClasses): ?>
                        (
                        <?php $__currentLoopData = $cartItem->options->taxClasses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $taxName => $taxRate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php echo e($taxName); ?> <?php echo e($taxRate); ?>%<?php if(!$loop->last): ?>, <?php endif; ?>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        )
                    <?php elseif(EcommerceHelper::isDisplayCheckoutTaxInformation() && $cartItem->taxRate > 0): ?>
                        (<?php echo e($cartItem->taxRate); ?>%)
                    <?php endif; ?>
                </small>
            </p>
        <?php endif; ?>
    ...
    <div class="col-auto text-end">
        <p class="mb-1 ec-checkout-item-price"><?php echo e($isCheckoutItemFree ? trans('plugins/ecommerce::ecommerce.free') : format_price($cartItem->price)); ?></p>
        <?php if(! $isCheckoutItemFree && $cartItem->qty > 1): ?>
            <p class="mb-0 ec-checkout-item-total">
                <small class="text-muted"><?php echo e(__('Total')); ?>: <?php echo e(format_price($cartItem->total)); ?></small>
            </p>
        <?php endif; ?>
    </div>
</div>
<?php /**PATH /var/www/dev.amatullah.bd/platform/plugins/ecommerce/resources/views/orders/checkout/product.blade.php ENDPATH**/ ?>